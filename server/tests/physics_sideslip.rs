...
fn run_sideslip_scenario(flow: Vec3f, ticks: usize, thrust: f32) {
    ...
    // right = up × forward
    let right = Vec3f { x: up.y * forward.z - up.z * forward.y, y: up.z * forward.x - up.x * forward.z, z: up.x * forward.y - up.y * forward.x };
...
fn run_rudder_sign_scenario(thrust: f32, _rudder: f32, warm_ticks: usize, steer_ticks: usize) {
    ...
    for i in 0..steer_ticks {
        ...
        if i + 100 >= steer_ticks {
            let forward = state.orientation * Vec3f::new(0.0, 0.0, 1.0); let up = Vec3f { x: 0.0, y: 1.0, z: 0.0 };
            let right = Vec3f { x: up.y * forward.z - up.z * forward.y, y: up.z * forward.x - up.x * forward.z, z: up.x * forward.y - up.y * forward.x };
            let v = state.velocity; let w = v.x * right.x + v.y * right.y + v.z * right.z; w_sum += w;
        }
    }
    let yaw1 = yaw_of(state.orientation);
    assert!(yaw1 > yaw0 + 0.01, "rudder right did not increase yaw (right-turn): yaw0={}, yaw1={}", yaw0, yaw1);
