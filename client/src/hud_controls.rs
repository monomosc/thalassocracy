...
impl Default for ThrustInput {
    fn default() -> Self { Self { value: 0.0, yaw: 0.0, pump_fwd: 0.0, pump_aft: 0.0, tick: 0 } }
}
...
fn ui_thrust_slider(mut egui_ctx: EguiContexts, mut thrust: ResMut<ThrustInput>, mut paused: ResMut<SimPause>) {
    use bevy_inspector_egui::egui::*;
    let Ok(ctx) = egui_ctx.ctx_mut() else { return; };

    SidePanel::left("thrust_panel").exact_width(90.0).show(ctx, |ui| {
        ui.heading("Controls");
        ui.add_space(8.0);
        ...
        let slider = Slider::new(&mut v, -1.0..=1.0).vertical().clamping(SliderClamping::Always);
        ui.add(slider);
        if (v - thrust.value).abs() > f32::EPSILON { thrust.value = v; }
        ui.add_space(6.0);

        ui.label("Rudder (+ = right)");
        let mut r = thrust.yaw;
        let slider_r = Slider::new(&mut r, -1.0..=1.0).vertical().clamping(SliderClamping::Always);
        ui.add(slider_r);
        if (r - thrust.yaw).abs() > f32::EPSILON { thrust.yaw = r; }
        ...
        let slider_pf = Slider::new(&mut pf, -1.0..=1.0).vertical().clamping(SliderClamping::Always);
        ui.add(slider_pf);
        if (pf - thrust.pump_fwd).abs() > f32::EPSILON { thrust.pump_fwd = pf; }

        ui.label("Pump AFT (+in)");
        let mut pa = thrust.pump_aft;
        let slider_pa = Slider::new(&mut pa, -1.0..=1.0).vertical().clamping(SliderClamping::Always);
        ui.add(slider_pa);
        if (pa - thrust.pump_aft).abs() > f32::EPSILON { thrust.pump_aft = pa; }

        ui.add_space(6.0);
        ui.monospace(format!("T {:.2} | R {:.2}\nPF {:.2} | PA {:.2}", thrust.value, thrust.yaw, thrust.pump_fwd, thrust.pump_aft));
    });
}

fn send_pause_request(client: Option<ResMut<RenetClient>>, paused: Res<SimPause>, mut last: Local<Option<bool>>) {
    let Some(mut client) = client else { return; };
    if !client.is_connected() { return; }
    let cur = paused.0;
    if last.map(|v| v == cur).unwrap_or(false) { return; }
