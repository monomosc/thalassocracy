...
fn fragment(@location(0) uv_in: vec2<f32>) -> @location(0) vec4<f32> {
    ...
    // Add a baseline so the effect is still visible at near depths (e.g. inside tunnels)
    // f = baseline + depth * (1 - baseline)
    let baseline = clamp(0.18 * params.x, 0.0, 0.7);
    let f = clamp(baseline + dist_n * (1.0 - baseline), 0.0, 1.0);
    ...
    let absorption = clamp(f * 1.8, 0.0, 1.0);
    var col = mix(base.rgb, refr.rgb, 0.65 * f);
    col = mix(col, diff, 0.35 * f * (0.3 + lum));
