...
pub enum CamMode { FirstPerson, Follow, Free }
...
pub fn update_game_camera(
    ...
    mut q_cam: Query<(&mut Transform, &FollowCam, &mut FollowCamState, &CamMode), (With<GameCamera>, Without<Submarine>)>,
) {
...
pub fn switch_cameras_keys(
    ...
    let Ok(mut mode) = q.single_mut() else { return; };
    if keys.just_pressed(KeyCode::F11) {
        *mode = if *mode == CamMode::Follow { CamMode::FirstPerson } else { CamMode::Follow };
    }
    if keys.just_pressed(KeyCode::F12) {
        *mode = if *mode == CamMode::Free { CamMode::FirstPerson } else { CamMode::Free };
    }
}
