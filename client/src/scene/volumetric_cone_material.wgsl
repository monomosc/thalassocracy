...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let emit = material_hdr_params.x * material_color.a * along * edge * view_boost * noise_mul * max(flicker, 0.0);
    // Visual alpha: keep in [0,1] to be well-behaved with transparency sorting
    let alpha_vis = clamp(material_color.a * along * edge, 0.0, 1.0);

    return vec4<f32>(base_rgb * emit, alpha_vis);
}
